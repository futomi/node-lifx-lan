...
LifxLanParser.prototype._parsePayload = function(type, pbuf) {
	...
	if(type === 3) { // StateService - 3
		...
		if(psize === 20) {
			payload = {
				build   : new Date(parseInt(pbuf.readUIntLE(0, 8) / 1000000, 10)),
		...
		if(psize === 24) {
			payload = {
				time     : new Date(parseInt(pbuf.readUIntLE(0, 8) / 1000000, 10)),
				uptime   : Math.round(pbuf.readUIntLE(8, 8) / 1000000), // msec
				downtime : Math.round(pbuf.readUIntLE(16, 8) / 1000000) // msec
			};
		}
		...
		if(psize === 56) {
			payload = {
				...
				updated : new Date(parseInt(pbuf.readUIntLE(48, 8) / 1000000, 10)) 
			};
		}

...
LifxLan.prototype.setColorBroadcast = function(params) {
	let promise = new Promise((resolve, reject) => {
		...
		} else if('x' in c && 'y' in c && 'brightness' in c) {
...
LifxLan.prototype._filtereDevices = function(params) {
	if(this._device_list === null || this._device_list.length === 0) {
		return {error: new Error('No device was found. Run the `discover()` method then try this method again.')};
	}
	// Check the `filters`
	let filter_list = [];
...
LifxLan.prototype._checkFilters = function(filter_list) {
	if(!Array.isArray(filter_list)) {
		return {error: new Error('The `filters` must be an Array object.')};
	}
	if(filter_list.length === 0) {
		return {filters: []};
	}
	...
	if(error) {
		...
		return {filters: new_filter_list};
	}
};
...
LifxLan.prototype._checkFilter = function(filter) {
	...
	if('label' in filter) {
		let v = filter['label'];
		if(typeof(v) !== 'string') {
			return {error: new Error('The `Filter.label` must be a string.')};
		}
		if(v) {
			o['label'] = v;
		}
	...
	if('productId' in filter) {
		...
		if(typeof(v) !== 'number' || v % 1 !== 0) {
			return {error: new Error('The `Filter.productId` must be an integer.')};
		}
		o['productId'] = v;
	}
	// Check the `features`
	if('features' in filter) {
		if(typeof(filter['features']) !== 'object') {
			return {error: new Error('The `Filter.features` is invalid.')};
		}
		let error = null;
		let features = {};
		['color', 'infrared', 'multizone'].forEach((k) => {
	...
	if('group' in filter) {
		if(typeof(filter['group']) !== 'object') {
			return {error: new Error('The `Filter.group` is invalid.')};
		}
		let group = {};
		if('guid' in filter['group']) {
			let v = filter['group']['guid'];
			if(typeof(v) !== 'string') {
				return {error: new Error('The `Filter.group.guid` must be a string.')};
			}
			if(v) {
				group['guid'] = v;
			}
		...
		if('label' in filter['group']) {
			...
			if(typeof(v) !== 'string') {
				return {error: new Error('The `Filter.group.label` must be a string.')};
			}
			if(v) {
				group['label'] = v;
			}
	...
	if('location' in filter) {
		if(typeof(filter['location']) !== 'object') {
			return {error: new Error('The `Filter.location` is invalid.')};
		}
		let location = {};
		if('guid' in filter['location']) {
			let v = filter['location']['guid'];
			if(typeof(v) !== 'string') {
				return {error: new Error('The `Filter.location.guid` must be a string.')};
			}
			if(v) {
				location['guid'] = v;
			}
		...
		if('label' in filter['location']) {
			...
			if(typeof(v) !== 'string') {
				return {error: new Error('The `Filter.location.label` must be a string.')};
			}
			if(v) {
				location['label'] = v;
			}

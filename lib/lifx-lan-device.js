...
LifxLanDevice.prototype._request = function (type, payload) {
	let promise = new Promise((resolve, reject) => {
		this._lifxLanUdp.request({
			...
			let payload = res && res['payload'] || null;
			resolve(payload);
		}).catch((error) => {
			reject(error);
		});
...
*   - length      | Integer | Optional | Affected tiles from/including tile_index (default: 1)
*   - x           | Integer | Optional | Sample from x position (default: 0)
*   - y           | Integer | Optional | Sample from y origin (default: 0)
*   - width       | Integer | Optional | Sample width/height size (default: 8)
* ---------------------------------------------------------------- */
...
*   - colors      | Array[Color]  | Required | Array of 64 Color objects (see: setColor())
* ---------------------------------------------------------------- */
...
* Method: tileGetTiles()
* ---------------------------------------------------------------- */
LifxLanDevice.prototype.tileGetTiles = function(params) {
	let promise = new Promise((resolve, reject) => {
...
* Method: tileSetTileState64(params)
* - params:
*   - color   | Object  | Required | Source HSBK object
*   - c       | Object  | Required | Color object values to merge/apply (see: setColor())
* ---------------------------------------------------------------- */

...
const inspect = _ => require('util').inspect(_, false, null, true)

const dump = _ => console.log(inspect(_))
...
LifxLanDevice.prototype._request = function (type, payload) {
	let promise = new Promise((resolve, reject) => {
		const params = {
			...
			ack_required: false,
			res_required: (payload && 'res_required' in payload) ? payload['res_required'] : true,
			target: this.mac
		}
		// console.log('REQUEST', inspect({ type, payload, params }))
		this._lifxLanUdp.request(params).then((res) => {
			// console.log('RESPONSE', inspect({ type, res }))
...
// c = to merge over existing color
